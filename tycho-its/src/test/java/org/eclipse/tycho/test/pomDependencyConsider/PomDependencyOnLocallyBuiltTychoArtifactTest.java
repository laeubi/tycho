...
public class PomDependencyOnLocallyBuiltTychoArtifactTest extends AbstractTychoIntegrationTest {
	...
	private static final GAV TEST_PROJECT_POM_DEPENDENCY = new GAV(
			"tycho-its-project.pomDependencyConsider.p2Data.reuse", "pomDependencyConsider.p2Data.reuse.testDataBundle",
			"0.0.1");
	...
	public void testReuseOfP2MetadataForPomDependencyOnTychoArtifact() throws Exception {
		...
		// check that bundle and source bundle make it into the p2 repository - this
		// isn't obvious due to TYCHO-561
	...
	private void setUpBundleWithSourceBundle() throws Exception {
		...
		// other Tycho builds
		LocalMavenRepositoryTool localRepo = new LocalMavenRepositoryTool(new NoopFileLockService());
