...
public class ProjectHelper {
    ...
    public Xpp3Dom getPluginConfiguration(String pluginGroupId, String pluginArtifactId, String goal) {
        ...
        return getPluginConfiguration(pluginGroupId, pluginArtifactId, goal, currentProject, session);
    }

    public Xpp3Dom getPluginConfiguration(String pluginGroupId, String pluginArtifactId, String goal,
            MavenProject project, MavenSession mavenSession) {
        PluginExecution pluginExecution = getPluginExecution(pluginGroupId, pluginArtifactId, goal, project,
                mavenSession);
    ...
    public PluginExecution getPluginExecution(String pluginGroupId, String pluginArtifactId, String goal) {
        MavenSession session = legacySupport.getSession();
        if (session == null) {
            return null;
        }
        ...
        return getPluginExecution(pluginGroupId, pluginArtifactId, goal, currentProject, session);
    }
    ...
    public PluginExecution getPluginExecution(String pluginGroupId, String pluginArtifactId, String goal,
        ...
        for (Plugin plugin : plugins) {
            if (plugin.getGroupId().equals(pluginGroupId) && plugin.getArtifactId().equals(pluginArtifactId)) {
                if (goal != null) {
                    //first check if there is a goal scoped configuration
                    for (PluginExecution execution : plugin.getExecutions()) {
                        if (execution.getGoals().contains(goal)) {
                            return execution;
                        }
